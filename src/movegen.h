...
* Pasted and modified from Koivisto by Luecx
* 
...
namespace Seraphina
{
	...
	inline void setMoveType(Move& move, MoveType pt)
	{
		move |= (pt << 16);
	}

	inline void setCapPromo(Move& move, PieceType ct)
	{
		move |= (ct << 20);
	}
	...
	inline Move setMove(const Square& from, const Square& to, const PieceType& pt, const MoveType& mt, const PieceType& caporpromo)
	{
		Move move{ 0 }; // Initialize move's block of memory
...
public:
	...
	void generatebyPieceType(Board& board, Seraphina::MoveType mt, Bitboard& opts);
	void generateCastling(Board& board);
